...
| F-004 (P2) | `ACAO: *` on 404 | Confirmed live on 404s **and** 200s (all responses), but **zero CORS code exists in the repo** — it is Vercel platform behaviour. | **No app-level action possible** (documented). No `Access-Control-Allow-Credentials` is ever set, so browsers will never expose a credentialed response cross-origin; only public content is readable. Exposure: none. |
