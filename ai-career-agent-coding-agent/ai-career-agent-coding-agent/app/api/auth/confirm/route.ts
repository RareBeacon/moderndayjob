 ...
 * confirmed". Accounts created before auto-confirmation are confirmed
 * here on demand so their owners can sign in. The route is only reached
 * after the client proved the password (the failed sign-in attempt), so
 * it never unlocks an account for the wrong person.
 */
