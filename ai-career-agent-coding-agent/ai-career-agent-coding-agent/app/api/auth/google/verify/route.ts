...
/**
 * Google sign-in verification state machine.
 *
 * GET  : called right after the OAuth callback. Reports whether this google
 *        session still needs email verification; issues + emails a code
 *        automatically when none is active (the callback page then routes
 *        the user to /verify-email).
 * POST : { action: 'send' } resend (rate limited), or
 *        { action: 'confirm', code } verify a 6-digit code.
 *
 * The gate only ever applies to google-linked sessions; password accounts
...
export async function POST(req: Request) {
  ...
  if (!isGoogleLinkedUser(user)) {
    return Response.json({ error: 'NOT_APPLICABLE', message: 'Only Google accounts need this step.' }, { status: 400 });
  }

  const rate = await enforceRateLimit(`google-verify:${requestIp(req)}:${user.id}`, 12, '1 h');
  ...
  if (parsed.data.action === 'send') {
    ...
    void auditEvent({ action: 'GOOGLE_VERIFY_CODE_SENT', resource: 'auth', userId: user.id, outcome: 'allow' });
